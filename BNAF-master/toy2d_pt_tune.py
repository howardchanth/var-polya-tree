...
def train_density2d(model, sigmoid_layer, polyatree, optimizer, scheduler, args):
...
def plot_density2d(model, sigmoid_layer, polyatree, args, limit=4, step=0.01):
    ...
    prob = torch.cat(
        [
            torch.exp(compute_log_p_x(model, sigmoid_layer, polyatree,  x_mb)).detach()
...
def main():
    ...
    for tree_level in [2, 3, 4, 5, 6]:
        for lr in [1, 0.5, 0.1, 0.05, 0.01, 0.005]:
            ...
            optimizer = torch.optim.Adam(
                [{'params': model.parameters(), 'lr': args.learning_rate},
                 {'params': polyatree.parameters(), 'lr': args.pt_lr}],  amsgrad=True
            )
            ...
            if args.experiment == "density2d":
                train_density2d(model, sigmoid_layer, polyatree, optimizer, scheduler, args)
            ...
            print("Plotting..")
            if args.experiment == "density2d":
                plot_density2d(model, sigmoid_layer, polyatree, args)
            elif args.experiment == "energy2d":
