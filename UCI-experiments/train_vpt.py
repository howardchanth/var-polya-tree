    ...
    def f():
        if args.save:
            ...
            torch.save(
                ...
                os.path.join(args.path, "checkpoint.pt"),
            )
...
def load_flow(model, args):
    def f():
        print("Loading flow..")
        checkpoint = torch.load(os.path.join(args.load, "checkpoint.pt"))
        model.load_state_dict(checkpoint["model"])
...
def train(
    ...
    optimizer.swap()
    validation_loss = -torch.stack(
        [compute_log_p_x(model, sigmoid_layer, polyatree, x_mb).mean().detach() for x_mb, in data_loader_valid],
        -1,
    ).mean()
...
def main():
    ...
    parser.add_argument("--load", type=str, default="checkpoint/gas_layers1_h10_flows5_gated_2025-01-12-13-32-48/")
    ...
    for tree_level in [3, 4, 5, 6]:
        for lr in [1, 0.5, 0.1, 0.05, 0.01]:
            for decay in [1, 0.5, 0.1, 0.05, 0.01]:
                args.tree_level = tree_level
                args.decay = decay
                args.learning_rate = lr

                args.path = os.path.join(
                    "checkpoint",
                    "{}{}_layers{}_h{}_flows{}{}_pt{}_lr{}_decay{}_{}".format(
            ...
            print("Creating BNAF model and PolyaTree model with {} levels".format(args.tree_level))
            ...
            print("Creating optimizer with learning rate {}, decay {}".format(args.learning_rate, args.decay))
            optimizer = Adam(
