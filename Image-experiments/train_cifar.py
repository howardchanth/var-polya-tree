...
def main(args):
    ...
    while train:
        for _, data in enumerate(trainloader, 1):
            ...
            if total_iter % 1000 == 0:
                ...
                bit_per_dim = (mean_loss + np.log(128.) * full_dim) \
                            / (full_dim * np.log(2.))
    ...
    test_bit_per_dim = (mean_test_loss + np.log(128.) * full_dim) \
                  / (full_dim * np.log(2.))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('CIFAR10 NICE PyTorch implementation')
